...
router.post('/heartbeat/system/:system_ip', auth, function (req, res, next){
  ...
  if(Object.keys(req.body).length === 0){
    ...
    if(req.body.total_mem && req.body.avail_mem)
      setObj.$set['systems.$.mem_percent_used'] = ((Number(req.body.avail_mem.split(" ")[0])/(Number(req.body.total_mem.split(" ")[0])))*100).toFixed(2);;

    if(req.body.total_disk_space && req.body.avail_disk_space)
      setObj.$set['systems.$.disk_percent_used'] = ((Number(req.body.avail_disk_space.split(" ")[0])/(Number(req.body.total_disk_space.split(" ")[0])))*100).toFixed(2);;
